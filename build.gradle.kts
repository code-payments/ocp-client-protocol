...
// The published artifact is entirely generated protobuf, so there is nowhere for a
// consumer to read which contract it holds. This writes that in: the version Gradle is
// building under, and the upstream commit ocp.lock is pinned at. `sync-protos.sh --local`
// writes `commit: LOCAL`, so a composite-build consumer sees LOCAL with no extra handling.
abstract class GenerateContractInfo : DefaultTask() {
    @get:InputFile
    ...
    fun generate() {
        ...
        val commit = lock.readLines()
            .firstOrNull { it.startsWith("commit:") }
            ?.removePrefix("commit:")
            ?.trim()
            .orEmpty()
        require(commit.isNotEmpty()) { "${lock.name}: no 'commit:' line" }

        val dir = outputDir.get().asFile.resolve("com/codeinc/opencode/gen")
        dir.mkdirs()
        dir.resolve("OcpContractInfo.kt").writeText(
            """
            // Generated by the generateContractInfo Gradle task. Do not edit.
            package com.codeinc.opencode.gen

            public object OcpContractInfo {
                public const val VERSION: String = "${packageVersion.get()}"
                public const val PROTO_COMMIT: String = "$commit"

                public val isLocal: Boolean get() = PROTO_COMMIT == LOCAL_SENTINEL

                public val shortProtoCommit: String
                    get() = if (isLocal) PROTO_COMMIT else PROTO_COMMIT.take(SHORT_LENGTH)

                private const val LOCAL_SENTINEL: String = "LOCAL"
                private const val SHORT_LENGTH: Int = 8
            }
...
// Captured eagerly rather than through a provider: `version` is resolved at configuration
// time from RELEASE_VERSION, and reading `project` from a task action is not permitted.
val contractInfoVersion = version.toString()

val generateContractInfo = tasks.register<GenerateContractInfo>("generateContractInfo") {
    lockFile.set(layout.projectDirectory.file("ocp.lock"))
    packageVersion.set(contractInfoVersion)
    outputDir.set(layout.buildDirectory.dir("generated/sources/contractinfo/main/kotlin"))
}

kotlin.sourceSets.named("main") {
    kotlin.srcDir(generateContractInfo)
}
